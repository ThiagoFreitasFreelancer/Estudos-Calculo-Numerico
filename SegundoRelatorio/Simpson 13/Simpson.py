...
def som(fx,intervalo,h,divisoes):
    ...
    for i in range(1,int(b)):
        a=result(fx,float(eval(intervalo[0]))+2*i*h)+a
    return a
def som1(fx,intervalo,h,divisoes):
    a=0
    b=divisoes/2
    for i in range(1,int(b)+1):
        a=result(fx,float(intervalo[0])+(2*i-1)*h)+a
    return a
def trapezio(h,fx,intervalo,divisoes):
    r=(h/3)*(result(fx[0],intervalo[0])+result(fx[0],intervalo[1])+2*som(fx[0],intervalo,h,int(divisoes[0]))+4*som1(fx[0],intervalo,h,int(divisoes[0])))
    return r
